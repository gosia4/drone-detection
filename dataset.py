...
# wczytuje ścieżki do obrazów i etykiety z określonego katalogu zbioru danych.
# Tworzy listę ścieżek do obrazów oraz odpowiadających im etykiet dla każdego testu.
    # iteruje przez dostępne pliki z obrazami i wczytuje odpowiednie adnotacje, zapisując ścieżki do obrazów oraz adnotacje w final_annotations
    def load_annotations(self, dataset_type):
        ...
        # initialise list to store names of files that are read
        read_files = []
        ...
        for test_name in name_of_tests:
            ...
            for x in range(num_of_image_files):
                ...
                # pass iteration if there aren't enough image frames from past frames
                ...
                try:
                    ...
                    with open(annot_path_labels_test_file, 'r') as f:
                        ...
                        # append the name of the file to the list of read files
                        if len(annot_path_images_test_file_list) >= self.batch_frames:
                            read_files.append(annot_path_images_test_file_list[-self.batch_frames:])
                        # read_files.append(annot_path_images_test_file_list[-self.batch_frames:])
        ...
        # print the names of the files that are read
        # print("Files read during dataset loading:")
    ...
    def preprocess_true_boxes(self, bboxes):
        ...
        for bbox in bboxes:
            ...
            # Pad bbox_xywh to ensure it has the correct shape (4,)
            bbox_xywh = np.pad(bbox_xywh, (0, max(0, 4 - bbox_xywh.shape[0])), mode='constant')
            ...
            for i in range(3):
                ...
                print("Wymiary bbox_xywh_scaled:", bbox_xywh_scaled[i][np.newaxis, :].shape)
                print("bbox_xywh_scaled:", bbox_xywh_scaled[i][np.newaxis, :])
                print("Wymiary anchors_xywh:", anchors_xywh.shape)
...
#             # image = cv2.imread(image_paths[x], cv2.IMREAD_GRAYSCALE)
#
#             # preprocess image
#             image = transform_images(image[:], tuple(self.train_input_size))
#
#             # image = transform_images(image[:], self.train_input_size)
...
#     #     np.random.shuffle(self.annotations)
#     #     print("Annotations shuffled")
#     #
#     # def __next__(self):
#     #     with tf.device('/cpu:0'):
#     #         # Sprawdź, czy powinieneś przetasować dane
#     #         if self.batch_count == 0:
#     #             self.shuffle_annotations()
#     #
#     #         # Resetuj licznik partii na początku każdej epoki
#     #         if self.batch_count >= self.num_batchs:
#     #             self.batch_count = 0
#     #             self.shuffle_annotations()
#     def __next__(self):
#         with tf.device('/cpu:0'):
#             print("train_input_size shape:", self.train_input_size)
#             print("strides shape:", self.strides)
#
#             # Obliczenie output sizes dla YOLO_v3
#             self.train_output_sizes = [self.train_input_size[0] // stride for stride in self.strides]
#
#             # Inicjalizacja tablicy zer dla obrazów
#             batch_image = np.zeros((self.batch_size, self.train_input_size[0], self.train_input_size[1],
#                                      3 * self.batch_frames), dtype=np.float32)
#
#             # Inicjalizacja tablic zer dla etykiet
#             batch_label_sbbox = [np.zeros((self.batch_size, size, size, self.anchor_per_scale, 5 + self.num_classes),
#                                            dtype=np.float32) for size in self.train_output_sizes]
#             batch_label_mbbox = [np.zeros((self.batch_size, size, size, self.anchor_per_scale, 5 + self.num_classes),
#                                            dtype=np.float32) for size in self.train_output_sizes]
#             batch_label_lbbox = [np.zeros((self.batch_size, size, size, self.anchor_per_scale, 5 + self.num_classes),
#                                            dtype=np.float32) for size in self.train_output_sizes]
#
#             # Inicjalizacja tablic zer dla bounding boxes
#             batch_sbboxes = np.zeros((self.batch_size, self.max_bbox_per_scale, 4), dtype=np.float32)
#             batch_mbboxes = np.zeros((self.batch_size, self.max_bbox_per_scale, 4), dtype=np.float32)
#             batch_lbboxes = np.zeros((self.batch_size, self.max_bbox_per_scale, 4), dtype=np.float32)
#
...
#                     batch_image[num, :, :, :] = image
#                     for i in range(3):
#                         batch_label_sbbox[i][num, :, :, :, :] = label_sbbox[i]
#                         batch_label_mbbox[i][num, :, :, :, :] = label_mbbox[i]
#                         batch_label_lbbox[i][num, :, :, :, :] = label_lbbox[i]
#                     batch_sbboxes[num, :, :] = sbboxes
#                     batch_mbboxes[num, :, :] = mbboxes
#                     batch_lbboxes[num, :, :] = lbboxes
#
...
#     #         onehot[bbox_class_ind] = 1.0
#     #
#     #         # bbox_xywh = bbox_coor.reshape(-1, 1) * np.array([self.train_input_size[::-1]])
#     #         # bbox_xywh = bbox_coor.reshape(-1, 1) * np.array(self.strides).reshape(-1, 1)
#     #         # bbox_xywh = bbox_coor[:2].reshape(-1, 1) * np.array(self.strides).reshape(-1, 1)
#     #         bbox_xywh = np.concatenate((bbox_coor[:2], [1])) * np.array(self.strides).reshape(-1, 1)
#     #
#     #         # bbox_xywh[2:] *= self.train_input_size[::-1]  # Skalowanie szerokości i wysokości
#     #         # bbox_xywh[2:] *= np.array(self.train_input_size[::-1]).reshape(1, -1)  # Skalowanie szerokości i wysokości
#     #         # bbox_xywh[2:] *= np.array(self.train_input_size[::-1][:2]).reshape(1,
#     #         #                                                                    -1)  # Skalowanie szerokości i wysokości
#     #         # bbox_xywh[2:] = bbox_xywh[2:].reshape(1, 2) * np.array(self.train_input_size[::-1][:2]).reshape(1,
#     #         #                                                                                                 2)  # Skalowanie szerokości i wysokości
#     #         # bbox_xywh[2:4] = bbox_xywh[2:4].reshape(1, 2) * np.array(self.train_input_size[::-1][:2]).reshape(1, 2)
#     #         bbox_xywh[2:4] *= np.array(self.train_input_size[::-1])[:2]
#     #
#     #         bbox_xywh = bbox_xywh.reshape(1, -1)
#     #         bbox_xywh_scaled = 1.0 * bbox_xywh / np.array(self.strides).reshape(-1, 1)
#     #
#     #         iou = []
#     #         exist_positive = False
#     #
...
#     #                 xind, yind = np.round(bbox_xywh_scaled[i, 0:2]).astype(np.int32)
#     #                 label[i][yind, xind, iou_mask, :] = 0
#     #                 # label[i][yind, xind, iou_mask, 0:5] = 0
#     #                 label[i][yind, xind, iou_mask, 0:4] = bbox_xywh
#     #                 label[i][yind, xind, iou_mask, 4:5] = 1.0
#     #                 label[i][yind, xind, iou_mask, 5:] = onehot
#     #
...
#
#             # stores true x_norm, y_norm, w_norm, h_norm relative to image height and width
#             bbox_coor = bbox[1:] # tablica z danymi obietku
#             print("Kształt bbox_coor:", bbox_coor.shape)
#             train_input_size_np = np.array(self.train_input_size)
#             print("Kształt train_input_size:", train_input_size_np.shape)
#
#             # one hot encoding of classes
#             onehot = np.zeros(self.num_classes, dtype=float)
#             onehot[bbox_class_ind] = 1.0
...
#
#             # obtain true x, y, w, h based on train_input_size, problem jest z tym, że train_unput_size jest (640, 480), a nie 416 (nie jest jedną wartością)
#             bbox_xywh = bbox_coor * self.train_input_size
#             # bbox_xywh = bbox_coor * (self.train_input_size[1], self.train_input_size[0])
#
#             # obtain true scaled x, y, w, h for each stride
#             bbox_xywh_scaled = 1.0 * bbox_xywh[np.newaxis, :] / self.strides[:, np.newaxis]
#
